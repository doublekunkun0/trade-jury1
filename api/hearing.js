...
async function fetchOkxQuote(payload) {
  ...
  if (!item) throw new Error("OKX returned no quote payload");
  ...
  const fromPriceUsd = parseFloat(firstDefined(routerResult.fromToken?.tokenUnitPrice, item.fromToken?.tokenUnitPrice, "0"));
  const toPriceUsd = parseFloat(firstDefined(routerResult.toToken?.tokenUnitPrice, item.toToken?.tokenUnitPrice, "0"));
  const fromAmountUsd =
  ...
  const gasUsd = parseFloat(firstDefined(routerResult.tradeFee, routerResult.networkFee, item.tradeFee, item.networkFee, "0")) || 0;
  const priceImpactPct = normalizePct(firstDefined(routerResult.priceImpactPercentage, item.priceImpactPercentage, "0"));
  ...
  return {
    ...
    toolName: firstDefined(primaryRoute?.dexName, primaryRoute?.router, primaryRoute?.dexProtocol?.dexName, "OKX Aggregator"),
...
async function fetchLifiQuote(payload) {
  ...
  return {
    ...
    toAmountFormatted: formatTokenAmount(fromMinimalUnits(response.estimate?.toAmount || "0", payload.toToken.decimals)),
    fromAmountUsd,
    toAmountUsd,
    gasUsd,
    protocolFeeUsd,
...
function normalizeOkxError(error) {
  ...
  if (/50114|invalid authority/i.test(message)) return "当前 API key 没有 Web3 DEX 访问权限";
  if (/fetch failed|timeout|aborted/i.test(message)) return "无法连接 OKX Web3 API";
  return message;
}
...
function simulateVerdict(payload, quote) {
  ...
  const targetRiskScore = { stable: 92, major: 74, defi: 58, meme: 24 }[targetRisk];
  ...
  log.push({
    speaker: "意图解析",
    body: `识别为 ${payload.fromToken.symbol} -> ${payload.toToken.symbol}，金额约 ${trimNumber(payload.amount)} ${payload.fromToken.symbol}，风险模式为 ${riskMode === "conservative" ? "保守" : riskMode === "balanced" ? "平衡" : "激进"}`
  });

  log.push({
    speaker: "实时报价",
    body: `${quote.providerLabel} / ${quote.toolName} 返回实时报价：预计到手 ${quote.toAmountFormatted} ${payload.toToken.symbol}，综合成本 ${formatUsd(quote.totalCostsUsd)}，总执行损耗 ${formatPct(-dragPct)}`
  });

  if (chasing) {
    jury.push({ role: "动量法官", vote: "approve", title: "趋势可能还在，但不是低风险进场", body: "报价证明路径可成交，但这类目标更像顺势追价，不应把可成交误判为值得成交" });
  } else if (defensive) {
    jury.push({ role: "动量法官", vote: "reject", title: "继续持有高波动资产没有明显优势", body: "当前实时报价足够干净，更适合把波动敞口收缩到防守资产" });
  } else {
    jury.push({ role: "动量法官", vote: "approve", title: "允许小仓位跟踪，不支持一把梭", body: "方向并非完全错误，但只能支持试单级别的风险暴露" });
  }

  if (chasing) {
    jury.push({ role: "逆向法官", vote: "reject", title: "热度过高，盈亏比不够", body: "目标资产偏情绪驱动，逆向视角认为现在更接近拥挤交易而不是舒服买点" });
  } else if (defensive) {
    jury.push({ role: "逆向法官", vote: "approve", title: "现在收缩风险比赌反弹更合理", body: "在报价足够干净时先落袋，可以把错误成本压到最低" });
  } else {
    jury.push({ role: "逆向法官", vote: "approve", title: "回调参与优于追涨参与", body: "如果必须做，这种结构更适合用分批方式而不是追价方式" });
  }
  ...
  jury.push({ role: "风险法官", vote: riskVote, title: riskTitle, body: riskBody });
  ...
  jury.push({
    ...
    body: executionVote === "approve" ? "实时报价显示当前路径可以承受，作为执行建议是成立的" : "报价不是不能用，但成本和路径复杂度已经提醒我们要降低仓位"
  });
  ...
  log.push({ speaker: "风险法官", body: riskBody });
  log.push({ speaker: "执行法官", body: executionVote === "approve" ? "路径、费用和到手额都在可接受范围内" : "可以成交，但不该按重仓逻辑去成交" });
  ...
  return {
    ...
    routeAdvice,
    evidence: [
      { label: "执行质量", score: executionQualityScore, note: executionQualityScore >= 80 ? "路径健康" : executionQualityScore >= 55 ? "谨慎可做" : "质量偏弱", detail: `${quote.toolName} · ${quote.toAmountFormatted} ${payload.toToken.symbol}` },
      { label: "综合成本", score: costScore, note: dragPct < 0.8 ? "成本轻" : dragPct < 1.8 ? "成本可控" : "成本偏重", detail: `${formatUsd(quote.totalCostsUsd)} · ${formatPct(-dragPct)}` },
      { label: "目标风险", score: targetRiskScore, note: payload.toToken.stable ? "防守型" : targetRisk === "meme" ? "高波动" : "可试单", detail: `${payload.toToken.symbol} · ${payload.toToken.name}` },
      { label: "路径结构", score: routeScore, note: routeSteps <= 2 ? "路径简洁" : "路径较复杂", detail: `${routeSteps} 跳 · 价格影响 ${formatPct(priceImpactPct)}` }
    ],
    jury,
    log
  };
...
function formatTokenAmount(value) {
  ...
  if (!Number.isFinite(numeric)) return trimNumber(value);
  return new Intl.NumberFormat("en-US", { minimumFractionDigits: 0, maximumFractionDigits: numeric >= 1000 ? 2 : numeric >= 1 ? 4 : 6 }).format(numeric);
}
